...
module.exports = class GRPCClient {
	constructor({ route, connections, serviceName, ServiceStub, packageDefinition }) {
		...
		this.interceptors = [
			new ClientTracingInterceptor(this.route).interceptor,
			new ClientMetadataOptionsInterceptor().interceptor,
			new ClientChannelOptionsInterceptor().interceptor,
			new ClientTokenInterceptor().interceptor,
	...
	createStub(clientOptions, parentCall) {
		...
		const options = {
			...clientOptions,
			interceptors,
			// TODO: let service owners define this and register it in zk along with
			// the service data.
