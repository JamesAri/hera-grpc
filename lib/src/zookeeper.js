...
const {parseLoadOptions, formatLoadOptions} = require('./utils')
		...
		try {
			...
			for (const pf of protoFiles) {
				...
				service.routes[route] = {
					...
					...(loadOptions && { loadOptions: formatLoadOptions(loadOptions) })
				}
			}
						...
						this.#client.getData(`/services/${child}`, (err, data) => {
							...
							try {
								...
								for (const route in service.routes) {
									service.routes[route].loadOptions = parseLoadOptions(service.routes[route].loadOptions)
