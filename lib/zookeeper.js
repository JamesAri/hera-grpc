	...
	async register(serviceInfo, protoFiles, next) {
		...
		try {
			...
			for (const pf of protoFiles) {
				...
				if (loadOptions) {
					// proto file(s) loaded in json descriptor buffer
					delete loadOptions.includeDirs
				}

				for (const route of routes) {
					// TODO: handle this better:
					// routes here share the proto file and load options
					// we could save this just once and reference it (without leaving orphaned znodes)
					service.routes[route] = {
						serviceName,
