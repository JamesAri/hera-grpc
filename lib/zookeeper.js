...
module.exports = class ZooKeeper extends EventEmitter {
	...
	constructor({ config }) {
		...
		const { host, pathname } = new URL(this.config.zk.connection)
	...
	async register(serviceInfo, protoFiles, next) {
		...
		try {
			...
			for (const pf of protoFiles) {
				const { serviceName, route, buffer, loadOptions: lo } = pf
				debug(`Registering proto file | ${serviceName} | ${route}`)
						...
						this.#client.getData(`/services/${child}`, (err, data) => {
							...
							try {
								...
								for (const route in routes) {
									const protoInfo = routes[route]

									protoInfo.loadOptions = deserializeLoadOptions(protoInfo.loadOptions)
									...
									servicesByRoute[route].push({
										...
										protoZnode: protoInfo.protoZnode,
										serviceName: protoInfo.serviceName,
										loadOptions: protoInfo.loadOptions,
									})
								}
							} catch (error) {
