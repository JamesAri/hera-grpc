...
const {decodeLoadOptions, encodeLoadOptions} = require('./utils')
		...
		try {
			...
			for (const pf of protoFiles) {
				...
				service.routes[route] = {
					...
					...(loadOptions && { loadOptions: encodeLoadOptions(loadOptions) })
				}
			}
						...
						this.#client.getData(`/services/${child}`, (err, data) => {
							...
							try {
								...
								for (const route in service.routes) {
									service.routes[route].loadOptions = decodeLoadOptions(service.routes[route].loadOptions)
