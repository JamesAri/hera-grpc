...
module.exports = class ServiceClient extends EventEmitter {
	constructor(options = {}) {
		...
		this.retryMaxAttempts = config.retry.getService.maxAttempts
		this.retryDelay = config.retry.getService.delay
		...
		try {
			...
			try {
				...
				this.toRegister.push({
					routes,
					buffer: isInternal || getProtoJsonDescriptorBuffer(filename, loadOptions),
					serviceName,
					loadOptions,
					isInternal,
				})
