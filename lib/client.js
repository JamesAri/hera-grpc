...
module.exports = class ServiceClient extends EventEmitter {
	...
	registerService(route, filename, serviceName, handlers, loadOptions) {
		...
		debug(`Registering service | ${serviceName} | ${route} | ${filename}`)
		...
		if (loadOptions && 'includeDirs' in loadOptions) {
			this._error(`${serviceName} | includeDirs load option is not supported`)
			return
		}
		...
		if (!filename) {
			this._error(`${serviceName} | Proto file must be provided to register a service`)
			return
		}
		...
		try {
			...
			this.grpcServer.registerService(ServiceStub, remoteHandlers)
		} catch (error) {
		...
		debug(
			`Registered service to grpc server | ${serviceName} | ${route} | ${JSON.stringify(serializeLoadOptions(loadOptions))}`,
		)
	}
}
