...
module.exports = class ServiceClient extends EventEmitter {
	...
	_connectToZookeeper(next) {
		...
		this.zookeeper.once('connected', () => {
			debugzk('connected') // TODO
		})

		this.zookeeper.on('disconnected', () => {
			debugzk('disconnected') // TODO
		})
		...
		for (const [key, fn] of Object.entries(handlers)) {
			...
			remoteHandlers[key] = function () {
				...
				return fn(
					...
					// => allows for remote nested calls - we can trace calls, propagate deadlines/cancellations, ...
					{
	...
	async _getStub(route, clientOptions, parentCall = null) {
		...
		try {
			return grpcClient.createClient(clientOptions, parentCall)
		} catch (error) {
			throw new Error(
	...
	registerService(route, filename, serviceName, handlers, loadOptions) {
		...
		debug(`Registering service | ${serviceName} | ${route}`)
		debug(`Proto file(s):`, filename)
		debug(`Proto file load options:`, serializeLoadOptions(loadOptions))
