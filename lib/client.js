...
const { loadServiceFromFile, getProtoJsonDescriptorBuffer, retry } = require('./utils')
...
module.exports = class ServiceClient extends EventEmitter {
	constructor({ config }) {
		...
		this.remoteServices = new RemoteServices({ zookeeper: this.zookeeper })
	...
	connect() {
		...
		// If there are no services, its only step step 1) and then 6)
	...
	_createRemoteHandlers(handlers) {
		...
		for (const [key, fn] of Object.entries(handlers)) {
			...
			remoteHandlers[key] = function () {
				...
				return fn(
					// context used from within service handlers (as first parameter) when making
					// remote (nested) calls - allows us to trace calls, propagate deadlines, ...
					{
						callService: (route, cb) => _callService(route, cb, call)
					},
					...arguments
				)
			}
	...
	async _callService(route, cb, parentCall = null) {
		...
		try {
			// TODO: allow override
			await retry(config.retry.maxAttempts, config.retry.delay, async () => {
		...
		try {
			// note: its up to the user to close the client
			const client = grpcClient.createClient(parentCall)
			debug(`Client for service ${grpcClient.serviceName} with route ${route} created`)
			await cb(client)
		} catch (error) {
			this._error(`Error calling service ${grpcClient.serviceName} with route ${route}: ${error.message}`)
			return
		}
