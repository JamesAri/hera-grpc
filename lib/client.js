...
module.exports = class ServiceClient extends EventEmitter {
	...
	close() {
		...
		if (this.grpcServer.isListening()) {
			...
			logger.debug('Initiating gRPC server graceful shutdown')
	...
	connect() {
		...
		tracer.startActiveSpan('connectServiceClient', (span) => {
	...
	_listen(next) {
		this.grpcServer.listen(this.host, this.port, (error, boundPort) => {
			...
			this.zookeeper.register(serviceInfo, this.toRegister, (error, res) => {
				...
				debugEmit('registered', boundPort)
	...
	_wrapHandlers(handlers) {
		...
		for (const [rpcName, handler] of Object.entries(handlers)) {
			if (typeof handler !== 'function') {
				this._errorThrow(`Service handler ${rpcName} must be a function`)
				return null
			}
			if (rpcName === 'getStub') {
				this._errorThrow('Service handler name "getStub" is reserved')
				return null
			}
			remoteHandlers[rpcName] = function () {
				...
				parentCall.getStub = (route, clientOptions) => _getStub(route, clientOptions, parentCall)
	...
	registerService({ routes, serviceName, handlers, filename, loadOptions }) {
		if (this.connectCalled) {
			this._errorThrow('Cannot register services after connecting')
			return
		}
		...
		if (!this.host || this.port == null) {
			this._errorThrow('Host and port must be provided during initialization to register services')
			return
		}

		if (!serviceName) {
			this._errorThrow(`${routes} | Service name must be provided to register a service`)
			return
		}

		if (!routes) {
			this._errorThrow(`${serviceName} | Routes must be provided to register a service`)
			return
		}
		...
		if (!filename) {
			this._errorThrow(`${serviceName} | Proto file(s) must be provided to register a service`)
			return
		}
		...
		try {
			...
			this._errorThrow(`Error parsing proto files ${filename}: ${error}`)
			return
		}
		...
		try {
			...
			this._errorThrow(`Error registering service with routes ${routes}: ${error.message}`)
			return
		}
