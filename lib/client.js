...
module.exports = class ServiceClient extends EventEmitter {
	...
	registerService({ routes, serviceName, handlers, filename, loadOptions }) {
		...
		if (!serviceName) {
			this._error(`${routes} | Service name must be provided to register a service`)
			return
		}

		if (!routes) {
			this._error(`${serviceName} | Routes must be provided to register a service`)
			return
		}
		...
		try {
			...
			this._error(`Error registering service with routes ${routes}: ${error.message}`)
			return
		}
