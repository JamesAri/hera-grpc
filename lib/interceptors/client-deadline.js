...
module.exports = class ClientDeadlineInterceptor {
	...
	addDefaultDeadline() {
		if (
			...
			debug('Propagating parent call deadline')
			// note: if user specified deadline, we will override it,
			// because it wouldn't make sense to not propagate the parent deadline.
			delete this.options.deadline // alternatively use: this.options.parent.getDeadline()
			return
		}

		// user doesn't wish to set a deadline => infinite deadline
		if (this.options.deadline === null || (typeof this.options.deadline === 'number' && this.options.deadline <= 0)) {
		...
		if (!this.options.deadline) {
			...
			deadline.setSeconds(deadline.getSeconds() + config.timeouts.clientDeadline)
			this.options.deadline = deadline
			debug(`Adding default deadline of ${config.timeouts.clientDeadline} seconds: ${deadline}`)
		} else {
	...
	interceptor(options, nextCall) {
		...
		debug(`Deadline is ${this.options.deadline}`)
